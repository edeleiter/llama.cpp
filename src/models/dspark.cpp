...
// DSpark == DFlash + a Markov-head logit bias. The graph below is identical to
// DFlash; the markov head is applied outside the graph, as a bias on the draft
// logits in the speculative driver (common_speculative_impl_draft_dspark).
...
void llama_model_dspark::load_arch_tensors(llama_model_loader &) {
    ...
    // markov head: prev-token bias read raw (F32) by the driver
    markov_w1 = create_tensor(tn(LLM_TENSOR_MARKOV_W1, "weight"), { (int64_t) markov_rank, n_vocab }, 0);
    markov_w2 = create_tensor(tn(LLM_TENSOR_MARKOV_W2, "weight"), { (int64_t) markov_rank, n_vocab }, 0);
...
// DSpark Encoder: processes target model features through feature fusion layer
template <>
